...
test('signup → unteamed list → matching → committed teams', () => {
  ...
  if (commit.ok) {
    ...
    for (const id of ['1', '2', '3', '4']) {
      const p = await getParticipant(db, EV, id);
...
test('blocked users cannot re-signup and are excluded from matching', () => {
  ...
  const res = await upsertParticipant(db, 'test', EV, G, '1', validated.value);
...
test('public team create → join → capacity enforced', () => {
  ...
  const created = await createTeam(db, 'test', EV, G, 'Alpha Team', 'public', '1');
  ...
  signup('6');
  const over2 = await joinTeam(db, 'test', EV, '6', created.value.id, DEFAULT_FORM.teamSize);
...
test('private team join code round-trip', () => {
  ...
  signup('1');
  signup('2');
  const created = await createTeam(db, 'test', EV, G, 'Secret Squad', 'private', '1');
  assert.ok(created.ok);
  if (!created.ok) return;
  assert.ok(created.value.joinCode !== null);

  const wrong = await joinPrivateTeam(db, 'test', EV, '2', 'XXXXXX', DEFAULT_FORM.teamSize);
  assert.equal(wrong.ok, false);

  const right = await joinPrivateTeam(db, 'test', EV, '2', created.value.joinCode!, DEFAULT_FORM.teamSize);
  assert.ok(right.ok);

  const leave = await leaveTeam(db, 'test', EV, '2');
  assert.ok(leave.ok);
});
...
test('setTeammates validates ids, dedupes and persists', () => {
  ...
  const p = await getParticipant(db, EV, '1');
  assert.ok(p !== null);
});
