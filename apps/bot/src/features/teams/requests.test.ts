...
test('invite flow: wrong decider rejected, invitee accept joins, members updated', () => {
  ...
  signup('owner');
  signup('alice');
  const team = createTeam(db, actorOf('owner'), G, 'Alpha', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const invite = createInvite(db, actorOf('owner'), G, team.value.id, 'alice', DEFAULT_FORM.teamSize);
  assert.ok(invite.ok);
  if (!invite.ok) return;
  ...
  const members = listTeams(db, G).find((t) => t.id === team.value.id)!.members;
...
test('join request flow: owner decides, duplicates blocked while pending', () => {
  ...
  signup('bob');
  signup('carol');
  const team = createTeam(db, actorOf('owner'), G, 'Bravo', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const req1 = createJoinRequest(db, actorOf('bob'), G, 'bob', team.value.id, DEFAULT_FORM.teamSize);
  const req2 = createJoinRequest(db, actorOf('carol'), G, 'carol', team.value.id, DEFAULT_FORM.teamSize);
  assert.ok(req1.ok && req2.ok);
  if (!req1.ok) return;

  const bobAgain = createJoinRequest(db, actorOf('bob'), G, 'bob', team.value.id, DEFAULT_FORM.teamSize);
...
test('capacity is enforced at accept time, not just request time', () => {
  ...
  const team = createTeam(db, actorOf('owner'), G, 'Charlie', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const ids: number[] = [];
  for (const u of ['m1', 'm2', 'm3']) {
    const inv = createInvite(db, actorOf('owner'), G, team.value.id, u, DEFAULT_FORM.teamSize);
  ...
  const req = createJoinRequest(db, actorOf('late'), G, 'late', team.value.id, DEFAULT_FORM.teamSize);
  ...
  const invLate = createInvite(db, actorOf('owner'), G, team.value.id, 'late', DEFAULT_FORM.teamSize);
...
test('cancel flow: sender can cancel their own pending request, then re-request', () => {
  ...
  signup('owner');
  signup('dave');
  const team = createTeam(db, actorOf('owner'), G, 'Delta', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const req = createJoinRequest(db, actorOf('dave'), G, 'dave', team.value.id, DEFAULT_FORM.teamSize);
  assert.ok(req.ok);
  if (!req.ok) return;
  ...
  const reRequest = createJoinRequest(db, actorOf('dave'), G, 'dave', team.value.id, DEFAULT_FORM.teamSize);
...
test('users already in a team cannot be invited elsewhere', () => {
  ...
  signup('owner2');
  signup('member');
  const t1 = createTeam(db, actorOf('owner1'), G, 'Echo-1', 'public', 'owner1');
  const t2 = createTeam(db, actorOf('owner2'), G, 'Echo-2', 'public', 'owner2');
  assert.ok(t1.ok && t2.ok);
  if (!t1.ok || !t2.ok) return;

  const inv = createInvite(db, actorOf('owner1'), G, t1.value.id, 'member', DEFAULT_FORM.teamSize);
  assert.ok(inv.ok);
  if (!inv.ok) return;
  assert.ok(decideRequest(db, actorOf('member'), inv.value.id, 'accept', DEFAULT_FORM.teamSize).ok);

  const poach = createInvite(db, actorOf('owner2'), G, t2.value.id, 'member', DEFAULT_FORM.teamSize);
