...
async function signup(userId: string) {
  ...
  const res = await upsertParticipant(db, 'test', EV, G, userId, validated.value);
...
test('invite flow: wrong decider rejected, invitee accept joins, members updated', async () => {
  ...
  signup('owner');
  signup('alice');
  const team = await createTeam(db, actorOf('owner'), EV, G, 'Alpha', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const invite = await createInvite(db, actorOf('owner'), EV, G, team.value.id, 'alice', DEFAULT_FORM.teamSize);
  assert.ok(invite.ok);
  if (!invite.ok) return;

  // The owner cannot accept their own invite — only the invitee decides.
  const wrong = await decideRequest(db, actorOf('owner'), invite.value.id, 'accept', DEFAULT_FORM.teamSize);
  ...
  const again = await decideRequest(db, actorOf('alice'), invite.value.id, 'accept', DEFAULT_FORM.teamSize);
...
test('join request flow: owner decides, duplicates blocked while pending', async () => {
  ...
  signup('bob');
  signup('carol');
  const team = await createTeam(db, actorOf('owner'), EV, G, 'Bravo', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const req1 = await createJoinRequest(db, actorOf('bob'), EV, G, 'bob', team.value.id, DEFAULT_FORM.teamSize);
  const req2 = await createJoinRequest(db, actorOf('carol'), EV, G, 'carol', team.value.id, DEFAULT_FORM.teamSize);
  assert.ok(req1.ok && req2.ok);
  if (!req1.ok) return;

  const bobAgain = await createJoinRequest(db, actorOf('bob'), EV, G, 'bob', team.value.id, DEFAULT_FORM.teamSize);
  assert.equal(bobAgain.ok, false);

  // The requester cannot decide their own request — only the owner (target).
  const wrongDecider = await decideRequest(db, actorOf('bob'), req1.value.id, 'accept', DEFAULT_FORM.teamSize);
...
test('capacity is enforced at accept time, not just request time', async () => {
  ...
  const team = await createTeam(db, actorOf('owner'), EV, G, 'Charlie', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const ids: number[] = [];
  for (const u of ['m1', 'm2', 'm3']) {
    const inv = await createInvite(db, actorOf('owner'), EV, G, team.value.id, u, DEFAULT_FORM.teamSize);
    if (inv.ok) ids.push(inv.value.id);
  }
  const targets = ['m1', 'm2', 'm3'];
  for (let i = 0; i < ids.length; i++) {
    const acc = await decideRequest(db, actorOf(targets[i]!), ids[i]!, 'accept', DEFAULT_FORM.teamSize);
    assert.ok(acc.ok);
  }

  // Team is full (owner + 3, size 4) → join request rejected at creation.
  const req = await createJoinRequest(db, actorOf('late'), EV, G, 'late', team.value.id, DEFAULT_FORM.teamSize);
  ...
  const invLate = await createInvite(db, actorOf('owner'), EV, G, team.value.id, 'late', DEFAULT_FORM.teamSize);
...
test('cancel flow: sender can cancel their own pending request, then re-request', async () => {
  ...
  signup('owner');
  signup('dave');
  const team = await createTeam(db, actorOf('owner'), EV, G, 'Delta', 'public', 'owner');
  assert.ok(team.ok);
  if (!team.ok) return;

  const req = await createJoinRequest(db, actorOf('dave'), EV, G, 'dave', team.value.id, DEFAULT_FORM.teamSize);
  assert.ok(req.ok);
  if (!req.ok) return;

  const wrongCancel = await cancelRequest(db, actorOf('owner'), req.value.id);
  assert.equal(wrongCancel.ok, false);

  const cancel = await cancelRequest(db, actorOf('dave'), req.value.id);
  assert.ok(cancel.ok);

  const reRequest = await createJoinRequest(db, actorOf('dave'), EV, G, 'dave', team.value.id, DEFAULT_FORM.teamSize);
...
test('users already in a team cannot be invited elsewhere', async () => {
  ...
  signup('owner2');
  signup('member');
  const t1 = await createTeam(db, actorOf('owner1'), EV, G, 'Echo-1', 'public', 'owner1');
  const t2 = await createTeam(db, actorOf('owner2'), EV, G, 'Echo-2', 'public', 'owner2');
